...
**規則不是 100% 強制**。當當前需求與本檔規則衝突時，Claude **不得自行決定打破規則**，而是要：

1. **明確標出衝突點**：在 plan 或回應中列出本次修改會違反第幾條 / 哪一節（例：「會違反 §3 Snapshot 規則第 3 點：snapshot 不可單獨呼叫 `_gistSilentPush()`」）
2. **說明後果**：解釋打破規則後可能重現哪一個 v15.069-079 已修補的回歸點，或可能引發新的同步異常
3. **提出替代方案**（若有）：例如延後同步到下次 user-data push、改用其他 domain 帶上等
4. **交由使用者評估**：等使用者明確同意「這次破例 OK」才動 code；不要假設使用者默許
5. **記入 §11 例外日誌**：使用者同意破例後，Claude 必須在本檔 §11 寫一筆紀錄（日期 / 版本 / 違反節次 / 原因 / 使用者同意）

**Codex review 同樣處理**：若 Claude 已破例但 plan / commit message 沒列出衝突點，列為 finding。

**禁止靜默違反**：直接改 code 又不提衝突，等於把規則文件當裝飾，下次又會踩同樣的坑。

### 規則進化機制（同條被破 ≥ 3 次強制改規則）

**例外不是免費的**。當 §11 例外日誌中**同一節 / 同一條** bullet 被破例累計到：

- **第 2 次**：Claude 必須在當次回應中**主動指出**「§X 已破例 2 次，下一次將觸發強制改規則流程」，提醒使用者注意
- **第 3 次（含以上）**：**禁止再以例外方式處理**。Claude 必須改進入「規則修訂流程」：
  1. 在 plan 中**明確說明**「§X 已破例 3 次，目前的規則描述與實際需求不符，建議改寫為 ___」
  2. 提出**新規則文字**（具體寫出要改成什麼），讓使用者比對
  3. 若使用者同意 → 直接改本檔對應節次，並在 §11 標註「§X 已升級至新版，舊例外日誌封存」
  4. 若使用者不同意 → 改其他方向（不破例、不進這次改動），由使用者重新指引

**為什麼要這樣設計**：規則被破第一次可能是個案；被破 3 次代表「規則沒抓對使用者真實需求」，繼續破例只是把 debt 累積在文件上。**規則應該與實際工作習慣同步進化**，而不是當成神主牌敬而遠之。

**統計責任**：每次寫 plan / 處理同步相關需求時，Claude 應主動掃 §11 看是否接近 3 次門檻，提早提醒使用者「§X 規則可能要重新審視」，不要等踩到第 3 次才被動觸發。
...
## 9. v15.069→v15.081 已修補的回歸點（不可重蹈）
...
3. 掃 §11 例外日誌看當前要改的區域是否已累積破例次數，若有提醒使用者
4. 撰寫 plan 時明確列出本次修改命中哪幾條規則（哪幾條會被測到）
5. 若會違反規則，依 §0 流程處理（標出衝突 / 說明後果 / 提替代 / 交評估 / 記日誌）
6. Codex review 時對照本檔逐條檢視；若 Claude 沒遵守，直接列為 finding

---

## 11. 規則例外日誌（append-only）

> 格式：`日期 | 版本 | 違反節次 | 一句話原因 | 使用者同意`
>
> 規則：每次破例**必加一筆**；同一節累計 ≥ 3 次必須觸發 §0 規則修訂流程，不可繼續以例外處理。
>
> 統計提醒：每次掃這份日誌時，Claude 應同步在心中累計各節次次數；接近門檻時主動提醒使用者。
